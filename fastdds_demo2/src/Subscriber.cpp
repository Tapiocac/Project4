  ...
  public:
    ...
    void on_data_available(DataReader *reader) override {
      ...
      if (reader->take_next_sample(&hello_, &info) ==
        ...
        if (info.valid_data) {
          samples_++;
          std::cout << "Message: " << hello_.data() << std::endl;
        }
      }
    }

    std_msgs::msg::dds_::String_ hello_;
...
public:
  HelloWorldSubscriber()
      : participant_(nullptr), subscriber_(nullptr), topic_(nullptr),
        reader_(nullptr), type_(new std_msgs::msg::dds_::String_PubSubType()) {}
  ...
  bool init() {
    ...
    std::cout << "Type registered: " << type_.get_type_name() << std::endl;
    ...
    // dr_qos.reliability().kind = RELIABLE_RELIABILITY_QOS;       // Reliable
    // dr_qos.durability().kind = VOLATILE_DURABILITY_QOS;
    // dr_qos.history().kind     = KEEP_LAST_HISTORY_QOS;         // Keep last
    // dr_qos.history().depth    = 10;                            // Same depth
    // as ROS2 pub reader_ = subscriber_->create_datareader(topic_, dr_qos,
    // &listener_);
